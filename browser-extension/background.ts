...
const readApiHostFromConfigFile = () => {
    const url = chrome.runtime.getURL('./assets/config/config.json');
    return fetch(url)
        .then(response => response.json())
        .then(config => config.apiHost);
}

const redirectGoLink = (details) => {
    const golink = details.url.replace(/(.*)\:\/\/go\//, "");
    redirect(golink);
}


const redirectSearchLink = (details) => {
